...
class WPCFM_Helper {
	...
	function get_file_bundles() {
		...
		foreach ( $filenames as $filename ) {
			...
			if ( is_multisite() ) {
				...
				} elseif ( 'blog' . get_current_blog_id() != $filename_parts[0] ) {
					continue;
				}
			}
	 ...
	 *
	 * @param $data array
	 * @param $save_format boolean
	 *
	 * @return mixed
	 */
	static function convert_to_yaml( $data, $save_format = true ) {
