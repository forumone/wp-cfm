...
class WPCFM_Readwrite {
	...
	function __construct() {
		...
		if ( ! is_dir( $this->folder ) ) {
			if ( ! wp_mkdir_p( $this->folder ) ) {
				$this->error = __( 'Create ' . $this->folder . ' and grant write access', 'wpcfm' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			}
		} elseif ( ! is_writable( $this->folder ) ) {
			$this->error = __( 'The ' . $this->folder . ' folder is not writable', 'wpcfm' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		}
	}
	...
	function pull_bundle( $bundle_name ) {
		...
		foreach ( $bundles as $bundle_name ) {
			...
			if ( ! $dont_update_settings ) {
				$dont_update_settings = 'wpcfm' == $bundle_name;
			}
			if ( $dont_update_settings ) {
				continue;
			}
